...
/***  Allocate work space.  ***/
    ...
    /* status code if any of the allocations fail, will be reset afterwards */
    S->outcome = 9;
    /* Initialize so we don't accidentially free a stray pointer */
    fvec = diag = qtf = fjac = wa1 = wa2 = wa3 = wf = NULL;
    ipvt = NULL;

    if ((fvec = (double *) malloc(m * sizeof(double))) == NULL)
        goto deallocate;
    if ((diag = (double *) malloc(n * sizeof(double))) == NULL)
        goto deallocate;
    if ((qtf  = (double *) malloc(n * sizeof(double))) == NULL)
        goto deallocate;
    if ((fjac = (double *) malloc(n*m*sizeof(double))) == NULL)
        goto deallocate;
    if ((wa1  = (double *) malloc(n * sizeof(double))) == NULL)
        goto deallocate;
    if ((wa2  = (double *) malloc(n * sizeof(double))) == NULL)
        goto deallocate;
    if ((wa3  = (double *) malloc(n * sizeof(double))) == NULL)
        goto deallocate;
    if ((wf   = (double *) malloc(m * sizeof(double))) == NULL)
        goto deallocate;
    if ((ipvt = (int *)    malloc(n * sizeof(int)   )) == NULL)
        goto deallocate;

    /* everything went fine, set status code */
    S->outcome = 0;
